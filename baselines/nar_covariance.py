"""
Estimate sparse conditional covariance matrices via NAR node fusion and QCM.

Inputs: trained SGA graph models, NAR quantile / NAR-MLP models, QCM moment
CSVs, and feature tensors; CLI flags ``--lr``, ``--hidden``, ``--lag``,
``--cuda``, and ``--graph`` (``variance`` | ``mean`` | ``tail`` | ``median``).
Operations: extract a sparse graph from SGA, estimate off-diagonal covariances
for connected pairs by fusing nodes and re-inferring quantiles with NAR, fill
diagonals from QCM variances, and project correlations to the nearest PD
matrix (NCM).
Outputs: ``{project_path}/nar_{graph}_graph_cov/hidden_*_.../{date}_cov.npy``.
"""
...
def estimate_covariance(sender,
    ...
    for tau in tau_list:
        ...
        nar = NAR(feature_dim=P, cuda=(device == 'cuda')).to(device)
        nar.load_state_dict(torch.load(f'{nar_model_path}/{tau}/network_best.pth',
                                       map_location=device))
        nar.eval()
        with torch.no_grad():
            adj_mat = network.encode_graph(fused_feature)
            r_prev = fused_feature[:, -1, ret_col]
            network_output = nar(fused_feature[:, -1, :], adj_mat, r_prev)
...
def estimate_covariance_mat(adj_mat, 
    ...
    covariance_list = Parallel(n_jobs=n_jobs)(delayed(estimate_covariance)
                                              (sender, receiver, feature, tau_list,
                                               ...
                                               nar_model_path, mlp_model_path,
                                               diagonal_elements, ret_col,
                                               device_id)
...
if __name__ == '__main__':
    ...
    nar_model_path = f'{project_path}/nar_quantile_model/{model_label}'
    ...
    moment_path = f'{project_path}/nar_moment/{model_label}'
    mlp_model_path = f'{project_path}/nar_mlp_model/{model_label}'
    ...
    if args.graph == 'variance':
        result_df, mat_dict, K = infer(N=N,
                                       ...
                                       graph=True)
        cov_path = f'{project_path}/nar_sigma_graph_cov/{model_label}'    
        ...
        result_df, mat_dict, K = infer(N=N,
                                       ...
                                       graph=True)
        cov_path = f'{project_path}/nar_mean_graph_cov/{model_label}'
        ...
        result_df, mat_dict, K = infer(N=N,
                                       ...
                                       tensor_path=quantile_tensor_path,
                                       graph=True)
        cov_path = f'{project_path}/nar_tail_graph_cov/{model_label}'
        ...
        result_df, mat_dict, K = infer(N=N,
                                       ...
                                       tensor_path=quantile_tensor_path,
                                       graph=True)
        cov_path = f'{project_path}/nar_median_graph_cov/{model_label}'
    else:
