...
class NAR_mlp_agent(object):
    def __init__(self,
        ...
        self.SGA = SGA(individual_num=individual_num - 1,
                       ...
                       dropout=dropout,
                       K=K,
                       cuda=cuda).to(self.device)
        self.SGA.load_state_dict(torch.load(f'{graph_model_path}/{K}/network_best.pth',
                                           map_location=self.SGA.device))
        for param in self.SGA.parameters():
            param.requires_grad = False

        self.NAR = NAR(feature_dim, cuda).to(self.device)
        self.NAR.load_state_dict(torch.load(f'{NAR_model_path}/network_best.pth',
                                           map_location=self.NAR.device))
    ...
    def load_data(self,
        ...
        self.train_loader = DataLoader(dataset_train, batch_size=1, shuffle=True, num_workers=num_workers)
        dataset_test = SGA_Dataset(data_path, valid_time, end_time)
        self.test_loader = DataLoader(dataset_test, batch_size=1, shuffle=True, num_workers=num_workers)
    ...
    def fuse_node(self, cross_feature, cross_label):
        ...
        new_vertex = self.mlp.forward(torch.Tensor(selected_features).to(self.device))
        other_vertices = torch.Tensor(np.delete(cross_feature, [a, b], axis=0)).to(self.device)
        ...
        label = np.concatenate([np.ones([1, 1]) * new_label, other_labels], axis=0)
    ...
    def qr_loss(self, network_output, y, tau):
        ...
        return qr_vector.sum() / (self.individual_num - 1)
        ...
        for i in range(epoch):
            ...
            for j, (X, y) in enumerate(self.train_loader):
                ...
                for _ in range(self.batch_size):    
                    ...
                    with torch.no_grad():
                        adj_mat = self.SGA.encode_graph(fused_feature)
                        r_prev = fused_feature[:, -1, self.ret_col]
                        network_output = self.NAR(fused_feature[:, -1, :], adj_mat, r_prev)
                    ...
                    if mse_loss:
                        loss = F.mse_loss(network_output, fused_label) + lambda_ * L1_loss
                    else:
                        loss = self.qr_loss(network_output, fused_label, tau) + lambda_ * L1_loss

                    batch_loss += loss

                train_losses.append(batch_loss.item())

                self.update_params(self.optim, batch_loss, networks=[self.mlp], retain_graph=False)
            ...
            if i % 3 == 0:
                ...
                torch.save(self.mlp.state_dict(), os.path.join(self.log_dir, 'mlp_final.pth'))
        ...
        with torch.no_grad():
            ...
            for j, (X, y) in enumerate(self.test_loader):
                ...
                for _ in range(self.batch_size):
                    ...
                    if mse_loss:
                        loss = F.mse_loss(network_output, fused_label)
                    else:
                        loss = self.qr_loss(network_output, fused_label, tau)
